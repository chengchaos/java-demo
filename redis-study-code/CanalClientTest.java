...
public class CanalClientTest {
    ...
    public static void main(String args[]) {
        ...
        jedis = new Jedis();
        // 和canal服务器进行连接
        CanalConnector connector = CanalConnectors.newSingleConnector(new InetSocketAddress("192.168.97.100",
    ...
    private static void redisInsert(String tableName, List<Column> columns) throws JsonProcessingException {
        ...
        if (columns.size() > 0) {
            jedis.set(RedisSerializer.string().serialize(tableName + "::" + columns.get(0).getValue()), RedisSerializer.json().serialize(user));
        }
    }
    ...
    private static void redisUpdate(String tableName, List<Column> columns) throws JsonProcessingException {
        ...
        if (columns.size() > 0) {
            jedis.set(RedisSerializer.string().serialize(tableName + "::" + columns.get(0).getValue()),
                    RedisSerializer.json().serialize(user));
        }
    }
    ...
    private static void redisDelete(String tableName, List<Column> columns) throws JsonProcessingException {
        ...
        if (columns.size() > 0) {
            jedis.del(RedisSerializer.string().serialize(tableName + "::" + columns.get(0).getValue()));
        }
    }
